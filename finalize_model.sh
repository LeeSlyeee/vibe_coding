...
~/Library/Python/3.9/bin/mlx_lm.fuse \
  ...
  --save-path models/haruON-gemma-2b-v2-full-f16

# 2. 4비트 양자화 (Quantization)
# 병합된 모델을 모바일용 1.4GB 사이즈로 압축합니다.
echo "📦 [2/2] 4비트 양자화 중 (Quantizing)..."
~/Library/Python/3.9/bin/mlx_lm.convert \
  --hf-path models/haruON-gemma-2b-v2-full-f16 \
  --mlx-path models/haruON-gemma-2b-v2-full-4bit \
  -q \
  --q-bits 4

echo "✅ 모든 작업 완료! 4비트 모델 위치: models/haruON-gemma-2b-v2-full-4bit"
