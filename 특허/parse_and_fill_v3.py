...
for paragraph in doc.paragraphs:
    # First, transfer the current paragraph
    ...
    if text == "【청구항1】":
        # we want to wipe out the empty claim and inject our own!
        # Oh, if we just appended 【청구항 1】, the original text is still there!
        # Actually it's better to clear it so we don't have dupes.
        p.clear()
        
        # Inject all claims
        for claim in claims:
            # We must split "【청구항 1】" from the rest so we can highlight or just dump as text.
            match = re.match(r'(【청구항 \d+】)\s*(.*)', claim, re.DOTALL)
            if match:
                cp = new_doc.add_paragraph()
                cp.add_run(match.group(1)).bold = True
                
                # claim text may have internal spacing we need to preserve, but let's just make it a single text block
                new_doc.add_paragraph(match.group(2))
            else:
                new_doc.add_paragraph(claim)
        continue
    
    # We shouldn't process other claims from the template
    if re.match(r'【청구항[ \d]+】', text) and text != "【청구항1】":
        # clear other claims so they don't appear natively empty
        p.clear()
        continue
    if re.match(r'\(.*청구항.*\)', text): # like (독립 방법항)
        p.clear()
        continue
