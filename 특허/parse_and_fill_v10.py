...
def fix_korean_spacing(text):
    # Fix common Korean spacing issues caused by HTML line breaks
    # Removes space before Korean particles and punctuations
    text = re.sub(r' (을|를|이|가|은|는|와|과|의|에|에게|에서|로|으로|까지|부터|다|합니다|된다)\b', r'\1', text)
    text = re.sub(r' ,', ',', text)
    text = re.sub(r' \.', '.', text)
    text = re.sub(r' ;', ';', text)
    text = re.sub(r' \)', ')', text)
...
def extract_clean_lines(tag):
    ...
    return [l.strip() for l in text.split('[BR]') if l.strip()]
...
def get_text_for_heading(heading_text):
    ...
    while curr and curr.name not in ['h1', 'h2', 'h3']:
        if curr.name in ['p', 'ul', 'ol', 'div']:
            lines.extend(extract_clean_lines(curr))
...
def get_prior_art():
    ...
    while curr and curr.name not in ['h1', 'h2', 'h3']:
        extracted = extract_clean_lines(curr)
    ...
    while curr and curr.name not in ['h1', 'h2']:
        if curr.name == 'div' and 'claim' in curr.get('class', []):
            ...
            if claim_text_div:
                ...
                body_lines = extract_clean_lines(claim_text_div)
...
# Adjust default font to standard readable font (Malgun Gothic)
...
for paragraph in doc.paragraphs:
    ...
    if text_nospace == "【청구항1】":
        ...
        for claim_num, body_lines in claims:
            ...
            for index, line in enumerate(body_lines):
                ...
                line_p.paragraph_format.line_spacing = 1.6
                if index > 0:
                    line_p.paragraph_format.left_indent = Pt(15)
        continue
    ...
    if raw_text in content_dict:
        ...
        for line in lines:
            ...
            np.paragraph_format.line_spacing = 1.6
            np.paragraph_format.space_after = Pt(8)
            np.paragraph_format.left_indent = Pt(10)
            if line.startswith('- '):
                np.paragraph_format.left_indent = Pt(25)
    ...
    if matched_img_key:
        ...
        if os.path.exists(img_path):
            ...
            img_run.add_picture(img_path, width=docx.shared.Inches(5.8))

new_doc.save(out_path)
print("Complete format done v10")
