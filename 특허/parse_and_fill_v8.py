...
def get_text_for_heading(heading_text):
    ...
    while curr and curr.name not in ['h1', 'h2', 'h3']:
        if curr.name in ['p', 'ul', 'ol', 'div']:
            # To prevent nested tags like <strong> causing newlines but allowing <br> to work:
            for br in curr.find_all('br'):
                br.replace_with(' [NEWLINE] ')
            text = curr.get_text()
            for line in text.split('[NEWLINE]'):
                # Clean up multiple whitespaces into one
    ...
    while curr and curr.name not in ['h1', 'h2']:
        if curr.name == 'div' and 'claim' in curr.get('class', []):
            ...
            if claim_text_div:
                ...
                body_lines = []
                for line in raw_body.split('[NEWLINE]'):
                    line = re.sub(r'\s+', ' ', line).strip()
                    if line:
...
print("Docx generation strictly formatted.")
