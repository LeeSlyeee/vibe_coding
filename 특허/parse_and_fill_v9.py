...
# Replace <br> tags with newline BEFORE BeautifulSoup parses it to prevent DOM destruction or inline element splitting
html = re.sub(r'<br\s*/?>', '\n', html)
    ...
    while curr and curr.name not in ['h1', 'h2', 'h3']:
        if curr.name in ['p', 'ul', 'ol', 'div']:
            ...
            for line in raw_text.split('\n'):
                line = line.strip()
                # merge multiple spaces (but preserve single spaces)
    ...
    while curr and curr.name not in ['h1', 'h2']:
        if curr.name == 'div' and 'claim' in curr.get('class', []):
            ...
            if claim_text_div:
                ...
                body_lines = []
                # Keep lines exactly as broken by <br> and newlines in HTML
...
for paragraph in doc.paragraphs:
    ...
    if text_nospace == "【청구항1】":
        ...
        for claim_num, body_lines in claims:
            ...
            for index, line in enumerate(body_lines):
                ...
                # First line is preamble, following lines are elements ending with ";" or separated by <br>: let's indent them
                if index > 0:
                    line_format = line_p.paragraph_format
                    line_format.left_indent = Inches(0.4)
        continue
    ...
    if matched_img_key:
        ...
        if os.path.exists(img_path):
            ...
            # Insert image, roughly 6.0 inches width to fit standard A4 margins perfectly
            img_run.add_picture(img_path, width=docx.shared.Inches(6.0))

new_doc.save(out_path)
print("Docx generation strictly formatted and successfully preserved all content.")
