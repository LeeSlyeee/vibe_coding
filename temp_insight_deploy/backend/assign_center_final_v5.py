...
def run():
    try:
        ...
        for p in patients:
            ...
            # [Fix] Fill missing 'first_name' -> API requires non-empty name
            if not p.first_name:
                p.first_name = f"환자{p.username[-4:]}" # Use last 4 chars of username
                p.save()
                print(f"   -> Fixed Missing Name for {p.username} -> {p.first_name}")
            
            d_count = Diary.objects.filter(user=p).count()
            print(f"   - {p.username}: Center={p.center}, Diaries={d_count}, Name={p.first_name}")
