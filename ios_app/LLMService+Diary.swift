...
extension LLMService {
    ...
    func generateMindGuide(recentDiaries: String, weather: String, weatherStats: String?) async -> String {
        ...
        let prompt = """
        당신은 사용자의 지난 일기 기록과 오늘의 날씨를 분석하여 따뜻한 한 문장의 조언을 건네는 감정 케어 도우미 '마음온'입니다. 절대로 '이성희'라는 이름을 쓰지 마세요.
    ...
    func generateAdvice(diaryText: String) async -> String {
       ...
       let prompt = """
       당신은 다정한 감정 케어 도우미 '마음온'입니다. 절대로 '이성희'라는 이름으로 당신을 소개하지 마세요.
    ...
    func generateLightAnalysis(diaryText: String) async -> (String, String) {
        ...
        let prompt = """
        당신의 이름은 '마음온'입니다. 절대로 '이성희'라고 대답하지 마세요. 따뜻한 감정 케어 도우미로서
    ...
    func generateUnifiedAnalysis(diaryText: String) async -> (String, String, String) {
        ...
        let prompt = """
        당신의 이름은 '마음온'입니다. 절대 '이성희'라고 하지 마세요. 임상 심리 전문가 수준의 감정 분석 AI로서
